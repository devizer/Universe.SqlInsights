...
# TODO: npx -y @puppeteer/browsers install chromium@706915
# It installs into ~/chromium/linux-706915/chrome-linux/chrome
function Find-Puppeteer-Browsers() {
  $searchFolder1 = Combine-Path "$($ENV:HOME)" "Browsers"
  $searchFolder2 = "$($ENV:PUPPETEER_BROWSERS_ROOT)"
  $ret = @()
  foreach($searchFolder in @($searchFolder1, $searchFolder2)) {
    if (-not ([System.IO.Directory]::Exists($searchFolder))) { continue; }
...
if (Test-Path "D:\") { 
  $sqlMediaFolder = "D:\SQL-Media"; $sqlSetupFolder = "D:\SQL-Setup"; $sqlInstallTo = "D:\SQL"; $root_drive="D:"
} Else {
  $sqlMediaFolder = "C:\SQL-Media"; $sqlSetupFolder = "C:\SQL-Setup"; $sqlInstallTo = "C:\SQL"; $root_drive="C:"
}

if (-not ((Is-Microsoft-Hosted-Build-Agent) -or (Is-Build-Agent))) {
...
if ((Get-Os-Platform) -ne "Windows") { 
  Set-Var "PS1_REPO_DOWNLOAD_FOLDER" "$($ENV:HOME)/.cache/PS1 Repo Downloads" 
} Else {
...
Set-Var "SQLINSIGHTS_REPORT_FULLNAME" "$SQLINSIGHTS_REPORT_FULLNAME"
  ...
  Set-Var "ERGOFAB_TESTS_HISTORY_CONNECTIONSTRING" "Server=$sql_instance_name;Encrypt=False;Initial Catalog=SqlInsights Local Warehouse;Pooling = true;$sql_security_parameters;"
  $ERGOFAB_TESTS_REPORT_FULLNAME = Combine-Path "$($ENV:SYSTEM_ARTIFACTSDIRECTORY)" "ErgFab Tests Report.txt"
  Set-Var "ERGOFAB_TESTS_REPORT_FULLNAME" "$ERGOFAB_TESTS_REPORT_FULLNAME"
